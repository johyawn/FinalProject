...
namespace MyNamespace
{
    public class MyCharacterControl : MonoBehaviour
    {
        ...
        private float lastJumpTime = 0f; // Time when the last jump occurred
        private readonly float minJumpInterval = 0.50f; // Minimum time between jumps
        ...
        {
            // check jump imput
            if (!jumpInput && Input.GetKey(KeyCode.Space) && Time.time - lastJumpTime >= minJumpInterval)
            {
                jumpInput = true;
